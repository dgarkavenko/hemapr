...
void expandGeoTiffToBounds(const fs::path& inPath,
  ...
  const int extendLeft = std::max(0, -minCol);
  const int extendTop = std::max(0, -minRow);
  const int extendRight = std::max(0, maxCol - sw);
  const int extendBottom = std::max(0, maxRow - sh);
  fmt::print("Will extend GeoTIFF by: left {} px, right {} px, top {} px, bottom {} px.\n",
             extendLeft, extendRight, extendTop, extendBottom);
  fmt::print("New size: {} x {}\n", newW, newH);
...
int main() {
  ...
  if (expected) {
    ...
    if (getGeoTransformBounds(ds, xmin, xmax, ymin, ymax)) {
      ...
      const double tol = hasGt ? std::max(std::abs(gt[1]), std::abs(gt[5])) * 0.01 : 0.0;
      bool within =
        std::abs(xmin - expected->xmin) <= tol &&
        std::abs(xmax - expected->xmax) <= tol &&
        std::abs(ymin - expected->ymin) <= tol &&
        std::abs(ymax - expected->ymax) <= tol;
      if (within) {
        fmt::print("Valid: actual bounds match expected within tolerance.\n");
      } else {
